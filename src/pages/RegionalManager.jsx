...
function HarvestActivityReport({ zone }) {
  ...
  return (
    <Group title="📊 Rep Activity — your team">
...
function ApptConversion({ zone }) {
  ...
  const load = async (override) => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        const res = await fetch(LB_ORIGIN + 'zone-appt-conversion?zone=' + encodeURIComponent(zone) + '&' + q)
  ...
  const downloadCsv = () => {
    ...
    const repRow = (z, r) => [z, r.rep, r.level || '', r.harvAp, r.harvSl, r.compAp, r.compSl, r.btrAp, r.btrSl, r.appts, r.sales, r.harvAmt, r.compAmt, r.btrAmt, r.amt, r.harvPct, r.compPct, r.btrPct, r.pct, r.grossPct, r.resitAp, r.resitSl, r.resitPct, r.pendAp, r.pendPct, r.deadAp, r.avg, r.rb, r.rb_pct, r.ins, r.ins_pct]
          ...
          {data.reps.length === 0 ? (
            ...
            <div className="overflow-hidden rounded-lg border border-slate-200 bg-white text-slate-800">
                ...
                <table className="w-full whitespace-nowrap text-sm">
                  ...
                  <thead>
                    <tr className="border-t border-slate-200 bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500">
                      ...
                      <th className="px-2 py-1.5 text-right" title="No-sit recovery: of re-booked sits (a &quot;No Sit — Need to Reschedule&quot; that got back on the calendar), how many closed. Shown as sold / re-sat · %.">NSR</th>
                  ...
                  <tbody>
                    {data.reps.map((r) => {
                      ...
                      {open && (
                        <tr><td colSpan={23} className="bg-slate-50 px-4 py-2"><ApptDetail details={r.details} /></td></tr>
                      )}
                      </Fragment>
                      )
                    })}
                  ...
                  return (
                    <div key={r.rep} className="p-3">
                      ...
                      <div className="mt-1 flex flex-wrap gap-x-3 text-[11px] text-slate-500">
                        <span>Harvest {r.harvAp} apt · <b className="text-emerald-700">{r.harvSl}</b> sold{r.harvAp ? ` · ${r.harvPct}%` : ''}</span>
                        <span>IQ {r.compAp} apt · <b className="text-emerald-700">{r.compSl}</b> sold{r.compAp ? ` · ${r.compPct}%` : ''}</span>
                        <span>BTR {r.btrAp} apt · <b className="text-emerald-700">{r.btrSl}</b> sold{r.btrAp ? ` · ${r.btrPct}%` : ''}</span>
                        {r.resitAp ? <span className="text-amber-700">NSR {r.resitAp} re-sat · <b>{r.resitSl}</b> sold · {r.resitPct}%</span> : null}
                        {r.pendAp ? <span className="text-sky-700">Pending <b>{r.pendAp}</b> ({r.pendPct}%) still working</span> : null}
                        {r.grossPct !== r.pct ? <span className="text-emerald-700">Gross <b>{r.grossPct}%</b> (w/ credit denials)</span> : null}
                      </div>
              ...
              <div className="px-3 py-2 text-[10px] text-slate-400">
                Appts counted in the week they happen (inspection signings excluded); sales in the week they close. Each category shows appointments then sales (count). Harvest = harvested (DoorDispatcher map/self-gen) · IQ = company lead (Instant Quote/AI Bot/FB…) · BTR = back-to-retail (from an inspection). Each bucket % = that bucket's sales ÷ appts. Net % = funded sales ÷ appts (what's counted). Gross % = funded sales + credit denials ÷ appts (a credit denial is a sale they couldn't finance). NSR = no-sit recovery: of re-booked sits (a "No Sit — Need to Reschedule" that got back on the calendar and re-sat), how many closed — shown as sold / re-sat · %; it's an overlay. Pending = open deals still being worked (status Sit - Pending) with pending % of appts — a high pending % flags a rep slow to close. Avg $/Sale = approved estimate ÷ sales.
              </div>
            </div>
          )}
        </div>
