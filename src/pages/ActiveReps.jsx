...
// Fields that the /directory page exposes. Order = display order in
// the visibility modal. Single source of truth — matches the keys the
// list-directory-reps function reads from directory_hidden.
...
// "3 of 5 shown", "name only", etc. Surfaces the state in the rep row
// so admin doesn't have to open the modal to remember.
function directoryHiddenLabel(hidden) {
...
// Modal that opens when admin clicks "+ Add staff / management". Used
// for people who aren't going through training (HR, ops, leadership)
// but should appear in the team directory. Captures basic contact info
// plus the per-field privacy choices for /directory.
function AddStaffModal({ regionNames, onCancel, onSave }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-lg border border-slate-200 bg-white p-6 shadow-xl">
        ...
        <div className="mt-4 rounded-md border border-slate-200 bg-slate-50 p-3">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Check anything that should NOT appear in the shared directory. Name and (Junior/Senior/Non-field) badge are always visible unless hidden.
...
// Modal that opens when admin clicks "🔒 Directory" on an active rep
// row. Lets them toggle which fields appear in the shared directory.
function DirectoryVisibilityModal({ trainee, hidden, setHidden, sending, onCancel, onConfirm }) {
  function toggle(key) {
