...
export default function ProvisioningHub() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error: err } = await supabase
      .from('classes')
      .select(
        'id, region, week_start_date, week_end_date, day_2_it_notified_at, it_completed_at, locations(name), trainees!class_id(id, enrolled, left_company_at, company_email, email_assigned_at, attendance(attendance_date, confirmed))',
      )
...
function ClassCard({ cls }) {
  ...
  const enrolled = (cls.trainees || []).filter((t) => t.enrolled !== false && !t.left_company_at)
