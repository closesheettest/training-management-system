...
export default function NealPayCard() {
  ...
  const next = band ? BANDS[BANDS.indexOf(band) + 1] : BANDS[0]
  const toNext = next && Number.isFinite(next.min) ? Math.max(0, next.min - gross) : 0
        ...
        <>
          ...
          {next && toNext > 0 && (
            <p className="mt-2 text-xs text-slate-600">
              <b>{usd(toNext)}</b> more this week reaches <b>{next.label}</b> at {pct(next.rate)} — worth {usd(next.min * next.rate)} instead of {usd(override || GUARANTEE)}.
            </p>
          )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[11px] uppercase tracking-wide text-slate-400">
                  <th className="py-1 pr-4 font-semibold">Gross sales</th>
                  <th className="py-1 pr-4 font-semibold">Rate</th>
                  <th className="py-1 font-semibold">Override at the bottom of the band</th>
                </tr>
              </thead>
              <tbody>
                {BANDS.map((b) => {
                  const here = band === b
                  return (
                    <tr key={b.label} className={here ? 'bg-brand-navy/5 font-bold text-brand-navy' : 'text-slate-600'}>
                      <td className="py-1 pr-4">{here && <span className="mr-1">▸</span>}{b.label}</td>
                      <td className="py-1 pr-4 tabular-nums">{pct(b.rate)}</td>
                      <td className="py-1 tabular-nums">{usd(b.min * b.rate)}</td>
                    </tr>
                  )
                })}
              </tbody>
          ...
          <p className="mt-3 text-xs text-slate-500">
            No override on IRBADs — an IRBAD raises gross sales directly, so it is already counted in the figure above.
            Gross is the same week's contract total the Managers Pay report below uses.
          </p>
        </>
      )}
    </div>
